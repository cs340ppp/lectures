...
Patterns are matched top down, and you can use a variable as a "catch-all" pattern.
...
Pattern matching should be *complete* (or *exhaustive*); i.e., covering all possible input values.
...
*Best practice*: Endeavor to make your pattern matches complete. The compiler warning is your friend!

### Aside: Partial Functions

Some functions are *partial* by design -- i.e., they aren't defined for all inputs. We should document this and can fail with an explicit error message:
...
As-patterns let you bind a variable to a value while also pattern matching on its substructure(s).
...
Boolean "guards" can be used to select between multiple right-hand-sides in a single function equation.
...
The `where` clause binds `f` locally, making the code cleaner and more efficient.
...
**Important**: All these constructs define *expressions* — each evaluates to a value (which must have a consistent, static type). They are not statements!
...
**Key insight**: Pattern matching in function definitions is actually *syntactic sugar* for `case` expressions!
...
`let-in` bindings are expressions, so they can be used anywhere an expression is expected.
...
**Expressions**: `if-then-else`, `case`, and `let-in` are all expressions that evaluate to values
